...
<script type="text/javascript">
	...
	function checkUsername(){
		...
		if (xmlhttp != null) {
			...
			alert(username);
			// 3.打开连接		因为这个路径几乎不会变动了，所有浏览器容易进行缓存，然后影响用户提交数据的正确性，所以加上一个时间戳，解决缓存问题
			xmlhttp.open("POST", "${pageContext.request.contextPath}/user_findByUsername.action?time="+new Date().getTime()+"&username="+username, true);
			// 4.发送
			xmlhttp.send();
		} else {
						...
						<table>
							...
							<tr>
								...
								<tr>
									...
									<td>
										<span class="fieldSet">
											<input type="text" id="checkcode" name="checkcode" class="text" maxlength="4" autocomplete="off"><img id="checkImg" class="captchaImage" src="${pageContext.request.contextPath}/checkImg.action" onclick="change()" title="点击更换验证码">
										</span>
									</td>
								</tr>
							<tr>
